...
model_list %>% 
  filter(!str_detect(model_name, "\\.\\d+"), model_name != "bt_final", model_name != "gp_prior") %>%
  transmute(
...
<aside>The first two were covered in the previous post.</aside>
...
home <- bind_rows(
  matches %>% filter(round != 'Qualifying') %>% select(rider_id = winner_id, home = winner_at_home),
  matches %>% filter(round != 'Qualifying') %>% select(rider_id = loser_id, home = loser_at_home)
  ) %>%
...
<aside> Görtler et al. have a really nice introduction, along with helpful interactive plots [here](https://distill.pub/2019/visual-exploration-gaussian-processes/). </aside>
...
Since the model requires in excess of 100 GPs to be fit, I've made use of the Hilbert space approximation to GPs analysed by [Riutort-Mayol, et al](https://arxiv.org/pdf/2004.11408.pdf).
...
<aside>The reason for using $-\kappa$ rather than $+\kappa$ is to retain the convention that larger parameter values favour the winning rider.</aside>
...
<aside>By taking $\text{logit}(0.55)/0.2 \approx `r round(qlogis(0.8)/0.2,1)`$ and $\text{logit}(0.55)/0.2 \approx `r round(qlogis(0.8)/0.2,1)`$.</aside>
...
<aside>Home effects have been included for Japanese athletes, in orange.</aside>
